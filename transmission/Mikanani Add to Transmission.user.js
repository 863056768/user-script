...
function installButtonOnTable() {
    ...
    const observer = new MutationObserver((mutationsList, observer) => mutationsList.forEach(mutation => installButton(mutation.target.previousElementSibling)));
    document.querySelectorAll('.episode-expand').forEach(targetNode => observer.observe(targetNode, { attributes: true }));
